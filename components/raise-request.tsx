...
export default function RaiseRequest() {
  ...
  useEffect(() => {
    const getStudentDetails = async () => {
      if (keepItSafeContract) {
        const studentDetails = await keepItSafeContract.getStudentDetails(wallet.address);
  ...
  const handleSubmit = async () => {
    if (user?.email) {
      ...
      if (keepItSafeContract) {
        const tx = await keepItSafeContract.requestDocument(instituteAddress, selectedProject?.value)
      }
  ...
  const projects = [
    ...
    {
      title: "Degree",
      description: "A streaming service that offers a wide variety of award-winning TV shows, movies, anime",
      link: "",
      value: "degree"
    },
    {
    ...
    },
    {
      ...
      description: "Another multinational technology company that specializes in Internet-related services and products.",
      link: "",
      value: "lor"
    },
  ];
  ...
  return (
    <div className="h-[100vh] flex justify-center items-center flex-col">
      <div className="mb-[1%]">
        <h1 className="text-6xl">Hello, {studentDetails?.[0] ?? "Loading"}</h1>
        <h1 className="mt-10 text-3xl">Raise a document request here</h1>
        <div className="max-w-5xl mx-auto px-8 mt-5">
          <HoverEffect items={projects} onSelect={handleProjectSelect} />
        </div>
